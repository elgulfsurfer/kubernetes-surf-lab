...
"""
SurfLog Admin CLI
Run via: kubectl exec -it deployment/backend -- python3 /scripts/admin.py
"""
...
def list_buoys():
    print("\n── Buoys ──")
    buoys = get("/buoy-list") if False else post("/admin/buoys-list") if False else None
    # Use spots endpoint workaround — list via spots which include buoys
    print("  (fetch buoys by checking spots below)")
    list_spots()
...
def list_spots():
    print("\n── Spots ──")
    # Need a JWT to call /spots — use admin login workaround via DB direct not available
    # Best we can do is show a reminder
    print("  Tip: log into the frontend to see all spots assigned to your user.")
...
def trigger_scrape():
    print("\n── Manual Buoy Scrape ──")
    print("  Run this from your local terminal (not inside the pod):")
